...
std::string EmitHostStub(const Module& module) {
  ...
  out << "  return true;\n";
  out << "}\n\n";
  out << "bool BuildSchedulerVmArgBuffer(gpga::MetalRuntime* runtime,\n";
  out << "                               const gpga::MetalKernel& kernel,\n";
  out << "                               std::unordered_map<std::string, gpga::MetalBuffer>* buffers,\n";
  out << "                               std::string* error) {\n";
  out << "  if (!runtime || !buffers) {\n";
  out << "    if (error) {\n";
  out << "      *error = \"Scheduler VM argument buffer requires runtime and buffers\";\n";
  out << "    }\n";
  out << "    return false;\n";
  out << "  }\n";
  out << "  if (!kernel.HasBuffer(\"sched_vm_args\")) {\n";
  out << "    return true;\n";
  out << "  }\n";
  out << "  constexpr uint32_t kVmArgIdBytecode = 0u;\n";
  out << "  constexpr uint32_t kVmArgIdProcOffset = 1u;\n";
  out << "  constexpr uint32_t kVmArgIdProcLength = 2u;\n";
  out << "  constexpr uint32_t kVmArgIdCondVal = 3u;\n";
  out << "  constexpr uint32_t kVmArgIdCondXz = 4u;\n";
  out << "  constexpr uint32_t kVmArgIdCondEntry = 5u;\n";
  out << "  constexpr uint32_t kVmArgIdSignalEntry = 6u;\n";
  out << "  constexpr uint32_t kVmArgIdIp = 7u;\n";
  out << "  constexpr uint32_t kVmArgIdCallSp = 8u;\n";
  out << "  constexpr uint32_t kVmArgIdCallFrame = 9u;\n";
  out << "  constexpr uint32_t kVmArgIdCaseHeader = 10u;\n";
  out << "  constexpr uint32_t kVmArgIdCaseEntry = 11u;\n";
  out << "  constexpr uint32_t kVmArgIdCaseWords = 12u;\n";
  out << "  constexpr uint32_t kVmArgIdExpr = 13u;\n";
  out << "  constexpr uint32_t kVmArgIdExprImm = 14u;\n";
  out << "  constexpr uint32_t kVmArgIdDelayAssignEntry = 15u;\n";
  out << "  constexpr uint32_t kVmArgIdAssignEntry = 16u;\n";
  out << "  constexpr uint32_t kVmArgIdForceEntry = 17u;\n";
  out << "  constexpr uint32_t kVmArgIdReleaseEntry = 18u;\n";
  out << "  constexpr uint32_t kVmArgIdServiceEntry = 19u;\n";
  out << "  constexpr uint32_t kVmArgIdServiceArg = 20u;\n";
  out << "  constexpr uint32_t kVmArgIdServiceRetAssignEntry = 21u;\n";
  out << "  std::vector<gpga::MetalBufferBinding> bindings;\n";
  out << "  bindings.reserve(19);\n";
  out << "  auto add_binding = [&](const char* name, uint32_t index) -> bool {\n";
  out << "    auto it = buffers->find(name);\n";
  out << "    if (it == buffers->end()) {\n";
  out << "      if (error) {\n";
  out << "        *error = std::string(\"Missing buffer for scheduler VM arg: \") + name;\n";
  out << "      }\n";
  out << "      return false;\n";
  out << "    }\n";
  out << "    bindings.push_back({index, &it->second, 0});\n";
  out << "    return true;\n";
  out << "  };\n";
  out << "  if (!add_binding(\"sched_vm_bytecode\", kVmArgIdBytecode) ||\n";
  out << "      !add_binding(\"sched_vm_proc_bytecode_offset\", kVmArgIdProcOffset) ||\n";
  out << "      !add_binding(\"sched_vm_proc_bytecode_length\", kVmArgIdProcLength) ||\n";
  out << "      !add_binding(\"sched_vm_cond_val\", kVmArgIdCondVal) ||\n";
  out << "      !add_binding(\"sched_vm_cond_xz\", kVmArgIdCondXz) ||\n";
  out << "      !add_binding(\"sched_vm_cond_entry\", kVmArgIdCondEntry) ||\n";
  out << "      !add_binding(\"sched_vm_signal_entry\", kVmArgIdSignalEntry) ||\n";
  out << "      !add_binding(\"sched_vm_ip\", kVmArgIdIp) ||\n";
  out << "      !add_binding(\"sched_vm_call_sp\", kVmArgIdCallSp) ||\n";
  out << "      !add_binding(\"sched_vm_call_frame\", kVmArgIdCallFrame) ||\n";
  out << "      !add_binding(\"sched_vm_case_header\", kVmArgIdCaseHeader) ||\n";
  out << "      !add_binding(\"sched_vm_case_entry\", kVmArgIdCaseEntry) ||\n";
  out << "      !add_binding(\"sched_vm_case_words\", kVmArgIdCaseWords) ||\n";
  out << "      !add_binding(\"sched_vm_expr\", kVmArgIdExpr) ||\n";
  out << "      !add_binding(\"sched_vm_expr_imm\", kVmArgIdExprImm) ||\n";
  out << "      !add_binding(\"sched_vm_delay_assign_entry\", kVmArgIdDelayAssignEntry) ||\n";
  out << "      !add_binding(\"sched_vm_assign_entry\", kVmArgIdAssignEntry) ||\n";
  out << "      !add_binding(\"sched_vm_force_entry\", kVmArgIdForceEntry) ||\n";
  out << "      !add_binding(\"sched_vm_release_entry\", kVmArgIdReleaseEntry) ||\n";
  out << "      !add_binding(\"sched_vm_service_entry\", kVmArgIdServiceEntry) ||\n";
  out << "      !add_binding(\"sched_vm_service_arg\", kVmArgIdServiceArg) ||\n";
  out << "      !add_binding(\"sched_vm_service_ret_assign_entry\", kVmArgIdServiceRetAssignEntry)) {\n";
  out << "    return false;\n";
  out << "  }\n";
  out << "  uint32_t arg_index = kernel.BufferIndex(\"sched_vm_args\");\n";
  out << "  gpga::MetalBuffer arg_buffer;\n";
  out << "  if (!runtime->EncodeArgumentBuffer(kernel, arg_index, bindings, &arg_buffer, error)) {\n";
  out << "    return false;\n";
  out << "  }\n";
  out << "  (*buffers)[\"sched_vm_args\"] = std::move(arg_buffer);\n";
  out << "  return true;\n";
  out << "}\n\n";
  ...
  out << "  }\n\n";
  out << "  auto halt_it = buffers.find(\"sched_halt_mode\");\n";
  out << "  if (halt_it != buffers.end() && halt_it->second.contents()) {\n";
  out << "    auto* halt_mode = static_cast<uint32_t*>(halt_it->second.contents());\n";
  out << "    for (uint32_t gid = 0; gid < count; ++gid) {\n";
  out << "      halt_mode[gid] = GPGA_SCHED_HALT_NONE;\n";
  out << "    }\n";
  out << "  }\n\n";

  out << "  if (needs_scheduler) {\n";
  out << "    auto sim_start = profiler.Now();\n";
  out << "    if (!BuildSchedulerVmArgBuffer(&runtime, sched_kernel, &buffers, &error)) {\n";
  out << "      std::cerr << error << \"\\n\";\n";
  out << "      return 1;\n";
  out << "    }\n";
  out << "    std::vector<gpga::MetalBufferBinding> bindings;\n";
