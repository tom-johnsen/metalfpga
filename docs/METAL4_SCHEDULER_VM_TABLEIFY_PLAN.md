...
2) Tableify edge wait items (highest ROI) (done)
...
3) Tableify service fallbacks (done)
...
4) Tableify delay ids (done)
   - Replace `switch (delay_id)` with a delay table indexed by id.
   - Keep bounds checks and error handling identical.

5) Tableify wait condition ids (done)
   - Replace `switch (sched_wait_id[idx])` with a table mapping wait id -> VM
     cond id and call the VM cond evaluator.
   - Keep bounds checks and ready/blocked behavior identical.

6) Tableify repeat counts (done)
